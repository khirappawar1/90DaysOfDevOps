...
```bash
# Create a pod without a YAML file (imperative type)
...
Ans: When the image field is missing, Kubernetes gives this error:

Error from server (Invalid): error when creating "nginx-pod.yml":
Pod "nginx-pod" is invalid:
...
Write a manifest for a third pod with at least 3 labels (app, environment, team). Apply it and practice filtering. --
 (In belwo image created third pod with the labels app,environment,team)
...
Ans: 🎯 One-line Interview Answer
Standalone Pods are not self-healing because they lack a controller, whereas Deployments ensure Pods are recreated automatically.
